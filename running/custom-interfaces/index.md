...
S.T.A.R.K. ships `VoiceAssistant` as a ready-made IO layer, and subclassing it (see [Voice Assistant & Modes](https://stark.markparker.me/running/voice-assistant/index.md)) covers most customization needs, overriding a method to hook into an event, like updating a GUI when a response arrives. But if you want a fundamentally different IO layer, a GUI, a Telegram bot, an API, rather than voice at all, `VoiceAssistant` isn't the starting point. `CommandsContextDelegate` is.
...
This is the same "your own assembly function" path covered in [How to Run](https://stark.markparker.me/running/how-to-run/index.md), that page is the better starting point for choosing between `run()`, overrides, and a fully custom delegate like this.
...
These aren't built-in, they're illustrations of where a custom `CommandsContextDelegate` (paired with a matching input source) fits well, to spark ideas for your own. See [Project Ideas](https://stark.markparker.me/ecosystem/project-ideas/index.md) for more.
...
Type instead of speak, read instead of listen. Excellent for debugging, quick testing, or environments without audio. This is what `STARK_VOICE_CLI=1` already gives you on top of `VoiceAssistant`, see [Voice Assistant & Modes](https://stark.markparker.me/running/voice-assistant/index.md), but a dedicated text-only delegate (like the minimal example above) skips voice entirely instead of layering on top of it.
...
The canvas of possibilities is vast, bounded mostly by the IO source you wire up. GUI and HTTP-API interfaces aren't built into S.T.A.R.K. yet, see [Roadmap](https://stark.markparker.me/ecosystem/roadmap/index.md) if you want to build one.
