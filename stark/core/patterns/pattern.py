...
class Pattern:
    ...
    def add_parameter_type(
        cls, object_type: ObjectType, parser: ObjectParser | None = None
    ):
        from ..types import Object
        ...
        assert exists_type is None or exists_type.type == object_type, (
            f"Can`t add parameter type: {object_type.__name__} already exists"
        )
    ...
    async def match(
        self,
        string: str,
        objects_cache: dict[str, Object] | None = None,
        ...
        logger.debug(
            f'Starting looking for "{self._origin}" "{compiled=}" in "{string}"'
        )
        ...
        for match in initial_matches:
            ...
            new_match, parsed_parameters = await self._parse_parameters_for_match(
                command_substr, objects_cache, prefill=prefill
            )
            ...
            all_parameters = {
                **{
                    param.name: None for param in self.parameters.values()
                },  # all with optional
                **{
                    k: v
                    for k, v in parsed_parameters.items()
                    if v.parsed_obj is not None
                },  # set parsed values
            }  # TODO: include prefill as parsed parameters (consider) or refactor a little
    ...
    async def _parse_parameters_for_match(
        ...
        # Use external prefills if provided
        external_prefill = prefill or {}
        ...
        while True:
            ...
            # combine external prefills and already parsed substrings
            combined_prefill = {
                **external_prefill,
                **{
                    name: parameter.parsed_substr
                    for name, parameter in parsed_parameters.items()
                },
            }

            # re-run regex only in the current command_str
            compiled = self.compile(prefill=combined_prefill)
            new_matches = list(re.finditer(compiled, string))

            logger.debug(
                f'Re capturing parameters {string} prefill={combined_prefill} compiled="{compiled}"'
            )
            ...
            for group_name, param in self.group_name_to_param.items():
                ...
                logger.debug(
                    f"{group_name}: {v}\t{param is not None}\t{group_name not in combined_prefill}\t{new_match.start(group_name) != -1}\t{new_match.start(group_name)}\t{bool(v.strip() if v else '')}\t{v}"
                )
            ...
            match_str_groups = {
                ...
                if name not in combined_prefill
            ...
            logger.debug(
                f"Found re groups: {[(new_match.start(name), name, match_str_groups[name]) for name in sorted(match_str_groups.keys(), key=lambda name: (int(Pattern._parameter_types[self.group_name_to_param[name].type_name].type.greedy), new_match.start(name)))]}"
            )
            ...
            parameter_match = await self._parse_single_parameter(
                parameter_name,
                match_str_groups[parameter_name].strip(),
                parsed_parameters,
                objects_cache,
            )
    ...
    async def _parse_single_parameter(
        ...
        for cached_parsed_substr, cached_parsed_obj in objects_cache.items():
            ...
            try:
                object_matches = await parameter_type.pattern.match(
                    raw_param_substr, objects_cache
                )
                if not object_matches:
                    raise ParseError(
                        f"Failed to match object {parameter_type} from {raw_param_substr}"
                    )
    ...
    def _filter_overlapping_matches(
        ...
        for prev, current in zip(
            ...
            if prev.start == current.start or prev.end > current.start:  # if overlap
                filtered.remove(
                    min(prev, current, key=lambda m: len(m.substring))
                )  # remove shorter
        return filtered

    def compile(
        self, group_prefix: str = "", prefill: dict[str, str] | None = None
        ...
        for parameter in self.parameters.values():
            ...
            if parameter.name in prefill:
                arg_pattern = prefill[
                    parameter.name
                ]  # TODO: review wether re.escape is needed
            else:  # replace pattern annotation with compiled regex
                parameter.group_name = (
                    (group_prefix + "_") if group_prefix else ""
                ) + parameter.name
                arg_pattern = parameter_type.pattern.compile(
                    group_prefix=parameter.group_name
                ).replace("\\", r"\\")
            ...
            pattern = re.sub(
                arg_declaration, f"(?P<{parameter.group_name}>{arg_pattern})", pattern
            )
    ...
    def _get_pattern_parameter_annotation_regex(self) -> re.Pattern:
        ...
        return re.compile(
            r"\$(?P<name>[A-z][A-z0-9]*)\:(?P<type>[A-z][A-z0-9]*)(?P<optional>\?)?"
        )
    ...
    def _update_group_name_to_param(self):
        self.group_name_to_param = {
            param.group_name: param for param in self.parameters.values()
        }
