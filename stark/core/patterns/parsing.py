...
class PatternParser:
    ...
    async def parse_object(
        self,
        object_type: ObjectType,  # type name might work as well
        from_string: str,
    ...
    async def match(
        ...
        pattern: Pattern,
        string: str,
        preparsed_parameters: dict[str, ParameterMatch] | None = None,
    ) -> list[MatchResult]:
        ...
        for match in initial_matches:
            ...
            new_match, parsed_parameters = await self._parse_parameters_for_match(pattern, command_substr, preparsed_parameters or {})
    ...
    async def _parse_single_parameter(
        ...
        # parse the object
        try:
            object_matches = await parameter_reg_type.type.pattern.match(raw_param_substr)
