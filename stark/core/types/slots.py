...
class Slots(Object, ABC):
    ...
    ) -> str:  # TODO: consider to override def parse instead (if present) to keep did_parse free?
        ...
        for param in slots.values():
            ...
            for object_pattern_match in object_matches:
                try:
                    ...
                    logger.debug(
                        f"Slot parameter match ParseError: {e}; trying next match..."
                    )
            else:
                if param.optional:
                    logger.debug(
                        f"Failed to match optional slot parameter {param.name} from {string}"
                    )
                    parameter_match = ParameterMatch(
                        name=param.name,
                        # regex_substr=string,
            ...
            if parameter_match.parsed_substr:
                start_index = min(
                    start_index, from_string.index(parameter_match.parsed_substr)
                )
                end_index = max(
                    end_index,
                    from_string.index(parameter_match.parsed_substr)
                    + len(parameter_match.parsed_substr),
                )
                string = string.replace(parameter_match.parsed_substr, "")

        if (
            len(list(p for p in parsed_parameters.values() if p.parsed_obj is not None))
            < 1
        ):
            raise ParseError(
                f"{type(self)} At least one parameter must be matched, can't find any of {slots.keys()} in '{from_string}'"
            )
