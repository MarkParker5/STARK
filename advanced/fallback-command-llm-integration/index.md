...
The [`stark-ai`](https://stark.markparker.me/ecosystem/package-registry/index.md) package already ships LLM processors — a fallback agent, structured/search parsing, and LLM/embedding NER. Check [STARK-PLACE](https://stark.markparker.me/ecosystem/index.md) before rolling your own.
...
1. **Don't mark it `hidden=True`.** A `hidden=True` command is never added to the manager's command list at all, it only becomes reachable when explicitly offered via a `Response`'s `commands=[...]` (see [Commands Context](https://stark.markparker.me/core-concepts/commands-context/index.md)). A fallback needs to be reachable from anywhere, all the time, so it can't be hidden.
