...
2. `TextDelegate` just needs to satisfy `CommandsContextDelegate`, a full, runnable implementation (print responses to the terminal) is in [Custom IO & Context Delegate, A Minimal Custom Delegate](custom-interfaces.md#a-minimal-custom-delegate).
...
| **GUI** | Not yet built | A great first contribution, see [Custom IO & Context Delegate](custom-interfaces.md#gui) for the shape it would take, and [Roadmap](../ecosystem/roadmap.md) |
...
**Full control, implement `CommandsContextDelegate` yourself.** This is the protocol `VoiceAssistant` itself implements. Skip `VoiceAssistant` entirely and you control the whole IO loop, this is what step 3 above does. See [Custom IO & Context Delegate](custom-interfaces.md) for the protocol and worked example.

---

Building something with a custom IO layer? [Share it in Discussions](https://github.com/MarkParker5/STARK/discussions) or contribute it to [STARK-PLACE](../ecosystem/index.md), GUI and API interfaces especially are open ground.
