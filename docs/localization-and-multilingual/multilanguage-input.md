...
Each slice resolves the majority language of that span, not the sentence's overall `"en"`. That resolved language is what gets passed to `did_parse` for each parameter's type. See [Phonetic Dictionary](../tools/phonetic-dictionary.md) for matching names across languages and spellings.
