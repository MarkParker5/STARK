...
Want it wired into a full assistant? See [How to Run](https://stark.markparker.me/running/how-to-run/index.md).
...
Pass `speaker_model_url` to enable speaker-embedding extraction (used for cross-recognizer matching when running [multiple languages simultaneously](https://stark.markparker.me/running/voice-assistant/#multi-language-voice-setup), not yet used for diarization, but the data is captured).
...
This is exactly where a cloud STT backend (Whisper API, Google Speech-to-Text, Azure) or a different offline engine would plug in, and a great first contribution if one doesn't exist yet. See [Roadmap](https://stark.markparker.me/ecosystem/roadmap/index.md).
