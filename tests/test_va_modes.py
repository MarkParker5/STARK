...
async def test_background_command_with_waiting_mode(voice_assistant, autojump_clock):
    async with voice_assistant() as va:
        await va.speech_recognizer_did_receive_final_result("background min")
        ...
        va._responses[0].time -= timedelta(seconds=va.mode.timeout_before_repeat + 1)
...
async def test_background_waiting_needs_input(voice_assistant, autojump_clock):
    async with voice_assistant() as va:
        await va.speech_recognizer_did_receive_final_result("background needs input")
        ...
        for response in va._responses:
            response.time -= timedelta(seconds=va.mode.timeout_before_repeat + 1)
...
async def test_background_waiting_with_context(voice_assistant, autojump_clock):
    async with voice_assistant() as va:
        await va.speech_recognizer_did_receive_final_result("background with context")
...
async def test_background_waiting_remove_response(voice_assistant, autojump_clock):
    async with voice_assistant() as va:
        ...
        await anyio.sleep(0.2)  # allow to capture command response
        responses_cache = va._responses.copy()  # cache all responses
