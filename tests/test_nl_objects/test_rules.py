...
def permutations(
    pattern_str, words: str | list[str], match: bool, expected_tokens: dict[str, str] | None = None
) -> list[tuple[str, str, bool, dict[str, str]]]:
...
pattern_parser.register_parameter_type(StarObject)
pattern_parser.register_parameter_type(GreedyObject)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        *permutations(f"{all_unordered('red', 'green', 'blue')}", "red green blue", True),
        *permutations(f"{all_unordered('red', 'green', 'blue')}", "red green red", False),
        *permutations(f"{all_unordered('red', 'green', 'blue')}", "red blue", False),
        ...
        *permutations(f"{one_or_more_unordered('$h:Hours', '$m:Minutes', '$s:Seconds')}", ["12 h"], True, {"h": "12"}),
        ...
        (
            f"{all_unordered('$a:StarObject $b:StarObject $c:StarObject')}",
            "one foo two bar three baz",
            True,
            {"a": "one foo", "b": "two", "c": "bar three baz"},
        ),
        (
            f"{all_unordered('$a:GreedyObject $b:GreedyObject $c:GreedyObject')}",
            "one foo two bar three baz",
            True,
            {"a": "one foo", "b": "two", "c": "bar three baz"},
        ),
...
async def test_unordered_patterns(pattern_str, input_str, is_match, expected_tokens):
    ...
    print(f'Compiled: "{pattern_parser._compile_pattern(p)}"')
    matches = await pattern_parser.match(p, input_str)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "$slots:GreedySlots",
            "nostart one foo two bar three baz noend",
            True,
            "one foo two bar three baz",
            {"a": "one foo", "b": "two bar", "c": "three baz"},
        ),
