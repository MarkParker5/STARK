...
def test_get_device_null():
    response = client.get(f'/api/device/')
    assert response.status_code == 405
    assert response.json() == {'detail': 'Method Not Allowed'}

def test_get_device_404():
    response = client.get(f'/api/device/{id}')
    assert response.status_code == 404
    assert response.json() == {'detail': 'Not found'}

def test_delete_device_404():
    response = client.delete(f'/api/device/{id}')
    assert response.status_code == 404
    assert response.json() == {'detail': 'Not found'}

def test_create_device():
    response = client.post(f'/api/device', json = device)
...
def test_patch_device():
    ...
    response = client.patch(f'/api/device/{id}', json = device)
    assert response.status_code == 200
    assert client.get(f'/api/device/{id}').json() == device

def test_delete_device():
    response = client.delete(f'/api/device/{id}', json = device)
    assert response.status_code == 200
